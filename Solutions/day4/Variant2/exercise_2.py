...
def create_book(
        ...
        author: str | None = Body(embed=True),
        year: int | None = Body(embed=True),
        genre: str | None = Body(embed=True)
    ) -> Book:
    """Creates new book."""
    new_id = max(books.keys()) + 1 if books else 1
    new_book = Book(id=new_id, title=title, author=author, year=year, genre=genre)
    books[new_id] = new_book
    return new_book


@app.put('/books/{book_id}')
def update_book(book_id: int, title: str = Body(embed=True), author: str | None = Body(default=None, embed=True)) -> Book:
    """Updates book info."""
    if book_id not in books:
